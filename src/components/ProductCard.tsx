...
const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  const handleOrderClick = () => {
    const url = generateWhatsAppUrl(product.name, product.price);
  ...
  return (
    <div
      ref={cardRef}
      className={`card p-0 overflow-hidden transform transition-all duration-500 ${
    ...
    >
      <div className="relative aspect-square overflow-hidden bg-grey-100">
        ...
        />
        <div className="absolute top-3 right-3 bg-(--color-accent) text-(--color-text) px-3 py-1 rounded-full text-sm font-bold shadow-md">
